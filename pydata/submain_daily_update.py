...
if __name__ == '__main__':
    ...
    flag_parser.add_argument('--ubcf', dest='UBCF_only', action='store_true')
    flag_parser.add_argument('--no-ubcf', dest='UBCF_only', action='store_false')
    ...
    # STEP 4.0 user based CF - too computational expensive on cov compute. --> svd applied
    if UBCF_only == True:
        u_rs_pred = get_u_pred_map(updated_user_item_matrix, topNeighbor, rs_topItem)
        data_tab = pd.DataFrame(u_rs_pred)
        model_updating_time = "No rec"
        need_provide_iu_ID = "user_ID"
        tech_type = "user based recommendation"
